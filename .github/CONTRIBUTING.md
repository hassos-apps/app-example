...
4. Run linters locally:
   ```bash
   hadolint example/Dockerfile
   yamllint example/config.yaml example/build.yaml
   shellcheck example/rootfs/**/*.sh
   ```
5. Test locally with a Docker build:
   ```bash
   docker build -t example-test example/
   ```
...
Use the [issue templates](https://github.com/hassos-apps/app-example/issues/new/choose) to report bugs or request features.
